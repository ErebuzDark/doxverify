    ...
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--color-surface)', fontFamily: 'var(--font-body)' }}>
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-6 py-24 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl mx-auto text-center">
          {/* Badge */}
          <div className="fade-up inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-[var(--color-border)] mb-8"
            style={{ background: 'rgba(255,255,255,0.04)' }}>
            <span className="w-1.5 h-1.5 rounded-full bg-[var(--color-success)]" style={{ animation: 'pulse-ring 2s ease-out infinite' }} />
            <span className="text-xs text-[var(--color-neutral-300)] tracking-widest uppercase">System Online</span>
          </div>

          <h1 className="fade-up-2 text-6xl md:text-7xl font-extrabold leading-tight mb-6"
            style={{ fontFamily: 'var(--font-display)' }}>
            <span className="shimmer-text">Verify.</span>
            <br />
            <span className="text-[var(--color-neutral-100)]">Trust. Confirm.</span>
          </h1>

          <p className="fade-up-3 text-lg text-[var(--color-neutral-400)] mb-14 max-w-xl mx-auto leading-relaxed">
            DoxVerify is a modern document authentication platform. Enter an authentication code below to instantly verify the legitimacy of any registered document.
          </p>
          ...
          <div className="fade-up-4 max-w-xl mx-auto">
            ...
            <p className="mt-3 text-xs text-[var(--color-neutral-500)]">
              Try: <button onClick={() => { setCode('DIPLOMA1'); setTimeout(handleVerify, 100) }}
                className="text-[var(--color-accent)] underline underline-offset-2 cursor-pointer">DIPLOMA1</button>
              {' · '}
              <button onClick={() => { setCode('CERT-2024A'); setTimeout(() => navigate('/verify/CERT-2024A'), 100) }}
                className="text-[var(--color-accent)] underline underline-offset-2 cursor-pointer">CERT-2024A</button>
            </p>
          </div>
        </div>
        ...
        <div className="relative z-10 max-w-4xl mx-auto mt-24 w-full grid grid-cols-1 md:grid-cols-3 gap-4">
          {FEATURES.map((f, i) => (
            <div key={i} className="card border-[var(--color-border-subtle)] group hover:border-[var(--color-border)] transition-all duration-300"
              ...
              <h3 className="font-bold text-[var(--color-neutral-100)] mb-2 text-sm" style={{ fontFamily: 'var(--font-display)', letterSpacing: '0.02em' }}>
                {f.title}
              </h3>
              <p className="text-xs text-[var(--color-neutral-500)] leading-relaxed">{f.desc}</p>
            </div>
          ))}
        </div>
      </main>
      ...
      {/* Footer */}
      <footer className="border-t border-[var(--color-border-subtle)] px-8 py-5 flex items-center justify-between">
        <span className="text-xs text-[var(--color-neutral-600)]">© 2025 DoxVerify. All rights reserved.</span>
        <span className="text-xs text-[var(--color-neutral-600)]">Secure Document Authentication Platform</span>
      </footer>
    </div>
  )
}
